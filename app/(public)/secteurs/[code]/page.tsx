...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!path || path.kind !== 'sector') {
    return { title: 'Parcours sectoriel', robots: { index: false, follow: false } }
  }

  return {
    title: path.title,
    description: path.objective ?? undefined,
    alternates: { canonical: `/secteurs/${path.code.toLowerCase()}` },
    openGraph: {
      title: path.title,
      description: path.objective ?? undefined,
      url: `/secteurs/${path.code.toLowerCase()}`,
    },
  }
}
...
export default async function Page({ params }: Props) {
  const { code } = await params
  return <PathDetail code={code} kind="sector" />
}
