...
describe('WC-2B — what the player does with the contract', () => {
  ...
  it('entitled playback is still gated by the entitlement view, not by media columns', () => {
    expect(PLAYER).toMatch(/\.from\('my_course_access'\)/)
    expect(PLAYER).toMatch(/if \(!access\?\.has_access\)/)
  })
})
  ...
  it('no code writes a derived field: they are generated by the database', () => {
    for (const f of FILES)
      for (const c of DERIVED)
        expect(f.text, `${f.path} assigns ${c}`).not.toMatch(new RegExp(`${c}\\s*:\\s*(?!string|'protected'\\s*\\|)`))
  })
})
